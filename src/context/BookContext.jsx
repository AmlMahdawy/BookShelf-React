...
const BookContextProvider = ({ children }) => {
    ...
    const fetchDataByAuthor = useCallback(async(selectedAuthor) => {
        try{
            ...
            const res=await api.get(`http://localhost:3000/book/author/${selectedAuthor}`);
            setOriginalBooks(res.data); 
            setBooksPages(res.data);
        }
        catch(error) {
    ...
    const addReview=(async(comment,stars,bookId)=>{
        const res=await api.post('http://localhost:3000/book/add-review',{bookId,comment,stars})
        console.log(res)
    })
